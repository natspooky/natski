...
function reloadVPS(settings) {
	...
	for (let i = 0; i < length; i++) {
		...
		video[length - i - 1].parentNode.parentNode.insertBefore(
			video[length - i - 1],
			video[length - i - 1].parentNode,
		);
...
function volumeSVGdeactiveVPS(e) {
	let svg =
			document.getElementsByClassName('VPSsound')[e].children[0].children,
		len = svg.length;
	for (let i = 0; i < len; i++) {
...
function closeVolumeVPS(e) {
	document
		.getElementsByClassName('volumeContainer')
...
function toggleSettingsVPS(e) {
	...
	if (
		vpsSetting.classList.contains('setting') &&
		!document.getElementsByClassName('VPS')[e].paused
	) {
		setTimeout(() => {
			hideContVPS(e);
		}, 300);
	...
	document
		.getElementsByClassName('settingsPanel')
		[e].classList.toggle('active');
	vpsSetting.classList.toggle('setting');
}

function videoTimeVPS(x, e) {
	document.getElementsByClassName('VPS')[x].currentTime =
		document.getElementsByClassName('VPS')[x].duration * (e / 100);
}

function currentVideoTimeVPS(e) {
	let video = document.getElementsByClassName('VPS')[e],
		[min, sec] = formatTimeVPS(video.currentTime);
	document.getElementsByClassName('timeCurrent')[
		e
	].innerText = `${min}:${sec}`;
}
...
function formatTimeVPS(e) {
	let temp = e % (60 * 60),
		...
		secd = temp % 60,
		seconds = Math.floor(secd).toLocaleString('en-US', {
...
function setPlaybackVPS(e, x) {
	...
	document.getElementsByClassName('VPSplayback')[e].innerText =
		'playback: ' + x.innerText;
}
...
function playPauseVPS(e) {
	...
	if (video.paused) {
		...
		document
			...
			[e].classList.remove('active');
		document
			.getElementsByClassName('VPSsettings')
...
function hideContVPS(e) {
	document
		.getElementsByClassName('VPScontrolBox')
		[e].classList.add('VPShidden');
}
	...
	if (document.pictureInPictureElement) {
		...
		if (isIOS) {
			...
			if (!document.getElementById('VPSpopup')) {
				...
				popup.id = 'VPSpopup';
				popuptxt.innerHTML =
					"FireFox doesn't support Picture In Picture";
...
function cinemaVPS(e) {
	...
	if (container.classList.contains('full')) {
		...
		container.style.width = document
			.getElementsByClassName('VPS')
			[e].getAttribute('VPSwidth');
	} else {
...
function wrapVPSvideo(e) {
	...
	for (let i = 0; i < video.childElementCount; i++) {
		...
		video.children[i].type = `video/${videoSrc.slice(
			videoSrc.indexOf('.') + 1,
			videoSrc.length,
		)}`;
	}
...
function createVPScontrols(e, device) {
	...
	for (let x = 0; x < 8; x++) {
		...
		playbackbut.onclick = function () {
			setPlaybackVPS(e, this);
			toggleSettingsPageVPS(
				e,
				document.getElementsByClassName('VPSplaybackMenu')[e],
			);
		};
...
function loadsettingsmenuVPS(e) {
	...
	document
		.getElementsByClassName('VPSplaybackMenu')
		[e].classList.remove('active');
...
function addVPSfuntions(e, device, setting) {
	...
	video.onended = function () {
		showContVPS(e);
		document
			.getElementsByClassName('VPSplay')
			[e].classList.remove('active');
	};
	...
	if (!device) {
		playframe.onclick = function () {
			playPauseVPS(e);
		};
		volume.oninput = function () {
			adjustVolumeVPS(e, volume.value);
		};
		volumecont.onclick = function () {
			muteVPS(e);
			showContVPS(e);
		};
		volumecont.onmouseenter = function () {
			openVolumeVPS(e);
		};
		contentbar.onmouseleave = function () {
			closeVolumeVPS(e);
		};
		volumebox.onmouseenter = function () {
			openVolumeVPS(e);
		};
		...
		proginput.ontouchstart = function () {
			...
			pauseVPS(e);
			document.getElementsByClassName('VPSplay')[e].style.opacity = '0';
			document.getElementsByClassName('VPSsettings')[e].style.opacity =
				'0';
...
function VPScssLoader() {
	if (deviceCheckVPS() || iOSspecificVPS()) {
		VPSstyleCall(
			`https://natski.netlify.app/lib/ENCORE_DB/VPS/${VPS_settings.style}VPSmobile.css`,
			loadVPS,
		);
	} else {
		VPSstyleCall(
			`https://natski.netlify.app/lib/ENCORE_DB/VPS/${VPS_settings.style}VPS.css`,
			loadVPS,
		);
	}
}
	...
	mutations.forEach(function (mutation) {
		mutation.addedNodes.forEach(function (node) {
			if (
				...
				node.parentNode.classList.contains('VPScontainer')
			) {
				return;
			}
			if (node.tagName == 'VIDEO' && node.classList.contains('VPS')) {
				...
				!node.classList.contains('VPScontainer')
			) {
					...
					children: [
						{
							type: 'source',
							attributes: {
								type: ENCORE_MIME.getMIME(
									this.VPS.getAttribute('src'),
								),
							},
						},
					],
