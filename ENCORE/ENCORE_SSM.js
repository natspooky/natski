...
function loadSSM(SSMsettings) {
	...
	if (inputs[0].parentNode.parentNode.classList.contains('SSMsearch')) {
	...
	for (let i = 0; i < selections.length; i++) {
		selections[i].addEventListener('click', function () {
			...
			popup.classList.remove('SSMvisible');
		});
		...
		for (let x = 1; x < selections[i].children[0].childElementCount; x++) {
			selections[i].children[0].children[x].addEventListener(
				'click',
				function () {
	...
	for (let i = 0; i < inputs.length; i++) {
		inputs[i].addEventListener('mouseenter', function () {
			popup.classList.add('SSMvisible');
			popup.innerHTML = `<p>${this.getAttribute('ssminfo')}</p>`;
		});
		inputs[i].addEventListener('mousemove', function (event) {
			popupSystemSSM(popup, event);
		});
...
function selectChangeSSM(element) {
	element.parentNode.children[0].value = element.value;
	element.parentNode.children[0].innerText = element.innerText;
}

function selectionToggleSSM(element) {
	element.classList.toggle('active');
}

function selectionCloseSSM(element) {
	element.classList.remove('active');
}

function popupSystemSSM(popup, position) {
	popup.style.transform = `translate(${position.clientX + 20}px,${
		position.clientY + 20
	}px)`;
}
...
function searchSSM(e, settings) {
	...
	for (let i = 0; i < len; i++) {
		if (
			settings[i].children[0].innerText.toLowerCase().includes(text) ||
		...
		console.log(
			`${text} in ${settings[
				i
			].children[0].innerText.toLowerCase()} = ${settings[
				i
