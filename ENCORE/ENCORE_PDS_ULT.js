...
function inspectLoadPDSU(settings) {
	...
	inspectimage.onload = function () {
		...
		if (settings.colourSystem) {
			inspectimage.style.backgroundColor = `${imageTintPDSU(
				inspectimage,
			)}0.8)`;
		}
	};
...
function animationStartPDSU(
	...
	let contained = getContainedSize(specificimage);
	inspectimage.style.transform = `translate(${
		left + (specificimage.offsetWidth - contained[0]) / 2
	}px,${top + (specificimage.offsetHeight - contained[1]) / 2}px) scale(${
		specificimage.offsetHeight / inspectimage.offsetHeight
	}) rotate(0deg)`;
	...
	if (settings.controls) {
		...
		for (let i = 0; i < 2; i++) {
			...
			minipagebuttons.onclick = function () {
				inspectChangePDSU(settings, page[i]);
			};
		...
		if (!PDSUdevice) {
			...
			container.addEventListener('touchmove', function (e) {
				...
				if (
					Math.abs(PDSUtouchMove - PDSUtouchStart) > 15 &&
					e.cancelable
				) {
					e.preventDefault();
					PDSUscrollLock = true;
...
function LoadMiniMenuPDSU(settings) {
	if (settings.controls) {
		...
		PDSUscale = 1;
		PDSUrotate = 0;
		miniviewer.children[1].innerHTML = `${PDSUimageIndex} / ${
			document.getElementsByClassName(settings.imageClass).length
		}`;
		if (!PDSUdevice) {
...
function checkImagePDSU(mutations) {
	...
	mutations.forEach(function (mutation) {
		mutation.addedNodes.forEach(function (node) {
			...
			if (
				node.tagName == 'IMG' &&
				node.classList.contains(settings.imageClass)
			) {
				let index = document.getElementsByClassName(
						settings.imageClass,
					),
					indexValue = Array.prototype.indexOf.call(index, node);
				if (settings.animations) {
					node.onclick = function () {
						inspectTargetAniPDSU(settings, indexValue + 1, this);
						LoadMiniMenuPDSU(settings);
					};
				} else {
					node.onclick = function () {
						inspectTargetPDSU(settings, indexValue + 1);
						LoadMiniMenuPDSU(settings);
					};
				}
...
function connectPDSU(settings) {
	...
	for (let i = 0; i < PDSlen; i++) {
		if (settings.animations) {
			PDSUimages[i].onclick = function () {
				inspectTargetAniPDSU(settings, i + 1, this);
				LoadMiniMenuPDSU(settings);
			};
		} else {
			PDSUimages[i].onclick = function () {
				inspectTargetPDSU(settings, i + 1);
				LoadMiniMenuPDSU(settings);
			};
		}
