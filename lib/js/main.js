...
if(window.location !== window.parent.location){
    parent.resetstatus(loadimage.length)
    loadimage.each(function(e) {
        $(this).imagesLoaded().done(function() {
            parent.currentstatus(loadimage.length)
        })
    })
}

$(document).ready(function() {
    window.location !== window.parent.location ? ($(".backpage").css("display", "flex")) : ($(".navbar").css("display", "block"), $(".backpage").css("display", "none"), $(".loaderback").css("display", "flex")), setTimeout(() => {
        $(".loaderback").fadeOut()
    }, 500);
    if(localStorage.getItem("performance")==='true'){
        let x=localStorage.getItem("performance")=='true';
        $(':root').css({'--accentcoltrans':'#1e1e1e','--blur':'0px'});
    }
    let e = new Date;
    document.getElementById("footerdate").innerHTML = "NATSKI<br>\xa9" + e.getFullYear() 
