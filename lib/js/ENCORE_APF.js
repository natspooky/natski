...
class APF {
    ...
    static squareRoot(number){
        ...
        for(let i = 0; i < 7; i++) {// Iterating while square_root is not equal to temp  //while (squareRoot != temp) {// aproximates to 13 loops but use 7 for accuracy
            squareRoot = (number / squareRoot + squareRoot) / 2// smalling the squareroot value to find square root
        }
        return squareRoot
    }
    static radiansToDegree(angle){
        return angle * (180 / 3.14)
    }
    static DegreeToRadians(angle){
...
//height of 1 width of 2
//collumn of 1 must = to row of two
//final matrix is row by column
    }
