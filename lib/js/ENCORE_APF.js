...
function isMobile(){
    return deviceCheckAPF() || iOSspecificAPF()
}


function max(array) {
    return sortArray(array)[array.length - 1]
}
...
function sortArray(array) {
    ...
    for(let i = 0; i < len; i++) {
        for(let x = 0; x < len - 1; x++){
            ...
            let tempStorage = NewArray[x];
            NewArray[x] = NewArray[x + 1]
            NewArray[x + 1] = tempStorage
            }
        }
...
function aproximateSquareRoot(value) {
    if (value < 0 || isNaN(value)) {//checking if number is negative or non-numeric
       return NaN
    }
    let squareRoot = value / 2,//starting the calculation from half of the number
    temp = 0
    for(let i = 0; i < 7; i++) {// Iterating while square_root is not equal to temp  //while (squareRoot != temp) {// aproximates to 13 loops
       temp = squareRoot
       squareRoot = (value / squareRoot + squareRoot) / 2// smalling the square_root value to find square root
    }
    return squareRoot
}
