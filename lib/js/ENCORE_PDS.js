...
function loadMiniImages(settings,currentimage) {
    let tileNum = 5,
    miniviewer = document.getElementById('miniview');
    miniviewer.innerHTML = ''
    for(let i = tileNum; i < 1; i--) {
        let imageTile = document.createElement('img');
        imageTile.className = 'imageTile'


        if(currentimage - i <= 0){
            imageTile.src = settings.path + (currentimage - i) + '.png'
            imageTile.dataset.tileNumber = (currentimage - i)
        }else{
        ...
        imageTile.onclick = function(e){inspectTarget(settings.path,e.target.dataset.tileNumber)}
