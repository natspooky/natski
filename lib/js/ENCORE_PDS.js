...
function PDSstyleCall(url) {
    PDScall.open('GET', url);
    PDScall.onloadend = PDSstyler;
    PDScall.send();
}
...
const PDSpromise = new Promise((resolve, reject) => {
      resolve(PDSstyleCall('https://natski.netlify.app/lib/ENCORE_DB/PDS/'+settings.style+'.txt'));
});