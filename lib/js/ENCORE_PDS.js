...
function reloadPDS(settings) {
    let len = document.getElementById('PDS').childElementCount
    for(let i = 0; i < len; i++) {
...
function createGalleryPDS(settings,PDS){
    ...
    for(let x = 0; x < rowNum; x++){//return value to 2
        let row = document.createElement('div')
        row.className = 'PDSrow'
        for(let i = 0; i < colNum; i++) {//return value to 2
...
function imageloadPDS(e, settings) {
    setTimeout(() => {
        e.style.opacity = '1'; 
        if(settings.colourSystem) {
            let tint = imageTintPDS(e)
...
function inspectLoadPDS(path) {
    ...
    inspectimage.onload = function() {
        if(inspectimage.clientWidth + 40 > document.getElementById('PDSUinspectPanel').offsetWidth){
            inspectimage.classList.add('active')
        }
        let theme = imageTintPDS(inspectimage)
...
function inspectLoadInitialPDS(path, speificimage) {
    ...
    inspectimage.style.opacity = '0'
    inspectimage.onload = function() {
        if(inspectimage.clientWidth + 40 > document.getElementById('PDSUinspectPanel').innerWidth){
            inspectimage.classList.add('active')
        }
...
function loadPDS(settings) {
    ...
    closer.onclick = function() {document.getElementById('PDSinspectPanel').classList.remove('active')}
