...
function createGallery(settings,PDS){
    ...
    for(let i = 0; i < images; i++){
        ...
        if(settings.animations == true) {
            display.onclick = function() {inspectTargetAni(settings.path, (i+1), this)}
        }else {
...
function loadPDS(settings) {
    ...
    container.appendChild(loadMiniImages(settings,miniviewer))
    for(let i = 0; i < 2; i++){
...
function loadMiniImages(settings,e) {
    let tileNum = 5,
    miniviewer = e;
