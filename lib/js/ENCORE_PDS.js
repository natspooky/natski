...
function inspectLoadInitial(path, speificimage) {
    ...
    inspectimage.onload = function() {
        ...
        setTimeout(() => {
            inspectimage.style.opacity = '1'
            inspectimage.style.transition = '0.6s cubic-bezier(.45,.13,.16,1.4), background-color 0s, opacity 0.1s, height 0.6s, width 0.6s'
            inspectimage.style.top = '50%'
            inspectimage.style.left = '50%'
...
if(checkLoadedPDS() == true) {
    if(PDSdevice){
        PDSstyleCall('https://natski.netlify.app/lib/ENCORE_DB/PDS/'+PDS_settings.style+'mobile.css', PDS_settings, loadPDS)
    }else{
