...
function format() {
    ...
    for(let i = 0; i < main.length; i++) {
        format += '<h3>' + main[i].children[1].value + '</h3><hr>'
        for(let e = 0; e < main[i].childElementCount - 4; e++) {
            for(let x = 0; x < 3; x++) {
                if(sub[e].children[1].value == value[x]) {
                    format += '<section>'+htmlData[x]
                }
            }
            format += '<p>' + sub[e + count].lastElementChild.value + '</p></section>'
        }
        count += (main[i].childElementCount - 4)
    }
