...
function changeScale(e){
    let tmpscale = scale,
    x = ((scale == 3) || (scale ==0.4) ? 0 : e)
    scale += e

    scale = Math.min(Math.max(scale,0.4),3)
    transformX = (transformX - (edit.offsetWidth / 2) + (mainimage.offsetWidth / 2)) * ((tmpscale/scale) * (1 + x)) + (edit.offsetWidth / 2) - (mainimage.offsetWidth / 2)
    transformY = (transformY - (edit.offsetHeight / 2) + (mainimage.offsetHeight / 2)) * ((tmpscale/scale) * (1 + x)) + (edit.offsetHeight / 2) - (mainimage.offsetHeight / 2)
    mainimage.style.margin = `${transformY}px 0px 0px ${transformX}px`
    mainimage.style.transform = `scale(${scale})`
    edit.style.backgroundSize = `${scale * 50}px`
    edit.style.backgroundPosition = `${transformX + (mainimage.offsetWidth / 2)}px ${transformY + (mainimage.offsetHeight / 2)}px`
