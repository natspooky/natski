...
function asignColour(){
    let colour = `${Math.floor(Math.random() * 255 ** 3).toString(16)}`
    colour = `#${Array(6 - colour.length).fill("0").join('') + colour}`
...
function generatePages(e){
    ...
    switch(e){
        case 'prev':
            document.getElementById('preview').children[0].innerHTML = 'right click to open!' + format
            reloadCMS(CMS_settings)
