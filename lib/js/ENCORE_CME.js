...
function generatePages(e){
    let format = '<div style="display: none; --CMScolour: ' + document.getElementById('CMScolour').value + '80;" id="CMS">' + generateMain() + generateSub() + '</div>'
    switch(e){
        case 'prev':
            document.getElementById('preview').children[0].innerHTML = format
            document.getElementById('CMS').style.display = 'block'
            document.getElementById('CMS').style.position = 'relative'
            document.getElementById('CMS').style.top = 'unset'
            document.getElementById('CMS').style.left = 'unset'
            CMSresetMenu()
            document.getElementById('CMS').classList.add('active')
            document.getElementById('CMS').style.height = document.getElementById('CMS').children[0].offsetHeight + 'px'
            break;
        case 'copy':
...
function generateParent(e,x){
    return '<button class="CMSclickable" onclick="CMSchangeMenu(`CMS'+ (getComputedStyle(x).getPropertyValue('--colour')).substring(1,7) +'`)">'+ generateImage(e.children[0]) +'<span>'+ e.children[0].children[5].getAttribute('value') +'</span><svg class="CMSsubpage" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 850.35 850.35"><path d="M816.99,312.17l-351.98,351.98c-22.01,22.01-57.69,22.01-79.7,0L33.32,312.17c-32.59-32.58-32.59-85.42,0-118h0c32.59-32.59,85.42-32.59,118,0L425.16,468.01,698.99,194.17c32.58-32.59,85.42-32.59,118,0h0c32.59,32.59,32.59,85.42,0,118Z"></path></svg></button>'
}
...
function generateBack(e,x) {
    if(x){
        return '<button class="CMSclickable CMSback" onclick="CMSresetMenu()"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 850.9 850.9"><path d="M367.73,771.63c-12.6,12.6-33.02,12.6-45.62,0L35.42,484.95l-18.57-18.57c-22.46-22.47-22.46-58.89,0-81.35l18.57-18.57L322.1,79.76c12.6-12.6,33.03-12.6,45.62,0l13.63,13.63c12.6,12.6,12.6,33.03,0,45.62l-231.7,231.7c-4.83,4.83-1.41,13.09,5.42,13.09h663.05c17.82,0,32.26,14.45,32.26,32.26v19.27c0,17.82-14.44,32.26-32.26,32.26H155.07c-6.83,0-10.25,8.26-5.42,13.09l231.71,231.7c12.6,12.6,12.6,33.03,0,45.62l-13.63,13.63Z"/></svg><span>Back</span></button>'
    }else{
        return '<button class="CMSclickable CMSback" onclick="CMSchangeMenu(`'+ e +'`)"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 850.9 850.9"><path d="M367.73,771.63c-12.6,12.6-33.02,12.6-45.62,0L35.42,484.95l-18.57-18.57c-22.46-22.47-22.46-58.89,0-81.35l18.57-18.57L322.1,79.76c12.6-12.6,33.03-12.6,45.62,0l13.63,13.63c12.6,12.6,12.6,33.03,0,45.62l-231.7,231.7c-4.83,4.83-1.41,13.09,5.42,13.09h663.05c17.82,0,32.26,14.45,32.26,32.26v19.27c0,17.82-14.44,32.26-32.26,32.26H155.07c-6.83,0-10.25,8.26-5.42,13.09l231.71,231.7c12.6,12.6,12.6,33.03,0,45.62l-13.63,13.63Z"/></svg><span>Back</span></button>'
    }
}
