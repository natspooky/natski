...
function animationStartPDSU(inspectimage, speificimage, left, top, playAnimationPDSU){
    ...
    playAnimationPDSU(inspectimage, windowLeft, windowTop)
}
        ...
        mutation.addedNodes.forEach(function(node) {
            ...
            if(node.tagName == 'IMG' && node.classList.contains(settings.imageClass)) {
                ...
                if(settings.animations) {
                    node.onclick = function() {inspectTargetAniPDSU(settings, (index), this);loadMiniImagesPDSU(settings,(index))}
                }else{
                    node.onclick = function() {inspectTargetPDSU(settings, (index));loadMiniImagesPDSU(settings,(index))}
                }
                node.style.cursor = 'pointer'
            }
            else if(node.childNodes.length > 0) {
                for(let children of node.childNodes) {
                    checkChildrenPDSU(children,settings)
                }
            }
        })
    })
...
function checkChildrenPDSU(element,settings){
    if(element.tagName == 'IMG' && element.classList.contains(settings.imageClass)) {
        let index = document.getElementsByClassName(settings.imageClass).length
        if(settings.animations) {
            element.onclick = function() {inspectTargetAniPDSU(settings, (index), this);loadMiniImagesPDSU(settings,(index))}
        }else{
            element.onclick = function() {inspectTargetPDSU(settings, (index));loadMiniImagesPDSU(settings,(index))}
        }
        element.style.cursor = 'pointer'
    }
    else if(element.length > 0) {
        checkChildrenPDSU(element.childNodes,settings)
    }
}
