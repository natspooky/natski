...
function inspectTargetAni(settings, indicator, speificimage) {
    document.getElementById('inspectPanel').classList.add('active')
    imageIndex = indicator
    inspectLoadInitial(settings, speificimage)
}

function inspectTarget(settings, indicator) {
    document.getElementById('inspectPanel').classList.add('active')
    imageIndex = indicator
    //document.getElementById('inspectImage').src = path + indicator + '.png';
    inspectLoad(settings)
}

function inspectLoad(settings) {
    ...
    inspectimage.src = PDSUimages[imageIndex-1].src;
}

function inspectLoadInitial(settings, speificimage) {
    let inspectimage = document.getElementById('inspectImage'),
    PDSUimages = document.getElementsByClassName(settings.imageClass),
...
function loadMiniImages(settings,currentimage) {
    ...
    if(PDSUimages.length > 1) {//change here!
        for(let i = (currentimage - 1); i <= (currentimage + 1); i++) {
            ...
            imageTile.onclick = function(){inspectTarget(settings,img);loadMiniImages(settings,img)}
...
function connectPDSU(settings) {
    ...
    for(let i = 0; i < PDSlen; i++){
        if(settings.animations) {
            PDSUimages[i].onclick = function() {inspectTargetAni(settings, (i+1), this);loadMiniImages(settings,(i+1))}
        }else {
            PDSUimages[i].onclick = function() {inspectTarget(settings, (i+1));loadMiniImages(settings,(i+1))}
        }
    }
}
