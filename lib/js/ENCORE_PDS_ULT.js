...
function inspectLoadPDSU(settings) {
    ...
    inspectimage.onload = function() {
        ...
        PDSUtransformX = (inspectPanel.offsetHeight / 2) - inspectimage.offsetHeight / 2,
...
function animationStartPDSU(inspectimage, speificimage, left, top, playAnimationPDSU){
    ...
    PDSUtransformX = (window.innerHeight / 2) - inspectimage.offsetHeight / 2,
    PDSUtransformY = (window.innerWidth / 2) - inspectimage.offsetWidth / 2
...
function LoadMiniMenuPDSU(settings) {
    ...
    if(settings.controls) {
        PDSUscale = 1
        miniviewer.children[1].innerHTML = `${PDSUimageIndex} / ${settings.fileCount}`
        if(!PDSUdevice){
