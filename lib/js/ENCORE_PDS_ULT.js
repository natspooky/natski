...
let PDSUdevice = deviceCheck();


function reloadPDSU(settings) {
    let len = document.getElementById('PDSU').childElementCount
    for(let i = 0; i < len; i++) {
        document.getElementById('PDSU').children[document.getElementById('PDSU').childElementCount -1].remove()
    }
...
function inspectLoad(path) {
    ...
    PDSUimages = document.getElementsByClassName(PDSU_settings.imageClass);
    ...
    inspectimage.onload = function() {
        console.log(inspectimage.clientWidth + 20, window.innerWidth)
        if(inspectimage.clientWidth + 40 > window.innerWidth){
            inspectimage.classList.add('active')
        }
        let theme = imageTint(inspectimage)
        inspectimage.style.transition = '0.4s, background-color 0s, opacity 0.4s'
        inspectimage.style.opacity = '1'
        inspectimage.style.backgroundColor = theme + "0.8)"
    }
    inspectimage.src = PDSUimages[imageIndex].src;
}
...
function inspectChange(settings, indicator) {
    let length = document.getElementsByClassName('PDSUimages').length;
    ...
    loadMiniImages(settings,imageIndex)
    inspectLoad(settings.path)
}
...
function loadPDSU(settings) {
    ...
    for(let i = 0; i < 2; i++){
        ...
        pagebuttons.onclick = function(){inspectChange(settings,page[i])}
    ...
    info.href = 'https://natski.netlify.app/encore%20alpha.html#PDSU'
...
function loadMiniImages(settings,currentimage) {
    ...
    if(settings.fileCount > 1) {//change here!
        for(let i = (currentimage - 1); i <= (currentimage + 1); i++) {
            ...
            imageTile.onclick = function(){inspectTarget(settings.path,img);loadMiniImages(settings,img)}
...
function connectPDSU(settings) {
    ...
    for(let i = 0; i < PDSlen; i++){
        if(settings.animations) {
            PDSUimages[i].onclick = function() {inspectTargetAni(settings.path, (i+1), this);loadMiniImages(settings,(i+1))}
        }else {
            PDSUimages[i].onclick = function() {inspectTarget(settings.path, (i+1));loadMiniImages(settings,(i+1))}
        }
    }
}
...
if(checkLoadedPDSU() == true) {
    if(PDSUdevice){
        PDSUstyleCall('https://natski.netlify.app/lib/ENCORE_DB/PDS/'+PDSU_settings.style+'PDSmobile.css', PDSU_settings, loadPDSU)
    }else{
        PDSUstyleCall('https://natski.netlify.app/lib/ENCORE_DB/PDS/'+PDSU_settings.style+'PDS.css', PDSU_settings, loadPDSU)
    }
}
