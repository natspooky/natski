...
function createGallery(settings,GVS){
    ...
    for(let x = 0; x < 2; x++){
        ...
        for(let i = 0; i < settings.fileCount; i++){
            ...
            display.className = settings.classes+ ' GVSimages'
...
function inspectChange(path, indicator) {
    if(imageIndex + indicator < 1){
        imageIndex = document.getElementsByClassName('GVSimages').length
    }else if(imageIndex + indicator > document.getElementsByClassName('GVSimages').length){
        imageIndex = 1
    }else{
        imageIndex += indicator
    }
