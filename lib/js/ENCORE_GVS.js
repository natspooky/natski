...
console.log('loading ENCORE_GVS')
let imageIndex = 0;
...
function createGallery(settings,GVS){
    ...
    for(let x = 0; x < 2; x++){
        ...
        for(let i = 0; i < settings.fileCount; i++){
            ...
            display.className = settings.classes+ ' inspectImages'
        ...
        for(let z = 0; z < settings.fileCount % 4; z++){
...
function inspectLoad(path) {
    ...
    inspectimage.onload = function() {
        setTimeout(() => {
            ...
            document.getElementById('inspectContainer').style.backgroundColor = theme + "0.8)"
        }, 100);
    }
}
...
function inspectChange(path, indicator) {
    if(imageIndex + indicator < 1){
        imageIndex = document.getElementsByClassName('inspectImages').length
    }else if(imageIndex + indicator > document.getElementsByClassName('inspectImages').length){
        imageIndex = 1
    }else{
        imageIndex += indicator
    }
    inspectLoad(path)
    console.log(document.getElementsByClassName('inspectImages').length, imageIndex,indicator,path + imageIndex + '.png')
}


function loadGVS(settings) {
    styleCall('https://natski.netlify.app/lib/ENCORE_DB/GVS/'+settings.style+'.txt')
    GVS = document.getElementById('GVS')
    createGallery(settings,GVS)
    ...
    imagecontainer.id = 'inspectContainer'
    ...
    for(let i = 0; i < 2; i++){
        ...
        pagebuttons.className = 'GVSbuttons'
        container.appendChild(pagebuttons)
    }
    GVS.appendChild(container)
...
const styler = function() {
    ...
    style.innerHTML = sysCall.responseText;
    document.getElementById('GVS').appendChild(style)
}

function styleCall(url) {
    sysCall.open('GET', url);
    sysCall.onloadend = styler;
    sysCall.send();
}

window.addEventListener("load", function() {
    loadGVS(GVS_settings)
    console.log('ENCORE_GVS loaded')
})
