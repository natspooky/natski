...
class calibrator extends VideoProcessor {
  ...
  while(super.getPosition(this.frame).length < 10){
    if(APF.sum(this.colour) > 0){
      ...
      trackerSettings.Threshold[this.threshold] += 1 * (counter / 10)
    }else{
...
function startCalibrate() {
  ...
  createTrackers()
  for(let i = 0; i < 3; i++) {
    console.log(125/trackerSettings.Threshold[i].value)
    document.getElementById(`Threshold${i+1}`).value = (125/trackerSettings.Threshold[i].value)
  }
}
