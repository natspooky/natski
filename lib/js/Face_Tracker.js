...
function setup(){
  ...
  for(let i = 0; i < 3; i++) {
    ...
    color[i] = 255;
    console.log(color)
    trackingPoints.push(new tracker(color, cameractx, PI))
  }
  //make thingy push classes into a list


    getMedia(videoConstraints).then(() => {getSettingsData()})
...
class VideoProcessor {
  ...
  space(color) {
    ...
    for(let i = 0; i < color.length; i++) {
      distance += (color[i] - this.colour[i]) * (color[i] - this.colour[i])
    }
...
class calibrator extends VideoProcessor {
  ...
  constructor(colour, colourIndex, videoFrame) {
    super(colour)
  ...
  while(super.getPosition(this.frame).length < 4){
    if(APF.sum(this.colour) > 0){
      ...
      trackerSettings.Threshold[this.threshold] += 4
    }
  }
  return this.colour
