...
function changeCameraData(value,constraintType){
    ...
    switch(constraintType){
      case 'device':
        videoConstraints.deviceId = value
        break;
      case 'fps':
        videoConstraints.frameRate = { max: Number(value) }
        trackerSettings.DrawTimer = 1000 / Number(value)
        break;
      case 'resolution':
        let tempValue = value.split(' ')
        videoConstraints.width = { max: Number(tempValue[0]) }
        videoConstraints.height = { max: Number(tempValue[1]) }
        break;
    }
    console.log(value)
...
function setup(){
  ...
  navigator.mediaDevices.getUserMedia({video:true,audio:true}).then(Info => {
  getMedia(videoConstraints)
  loadSettingsValues()
  })
}
