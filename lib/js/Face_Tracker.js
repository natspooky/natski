...
class trackingDisplay {
  ...
  calculateCenter(positionArray) {
    let center = {x:0, y:0};
    for(let i = 0; i < positionArray.length; i++) {
      center.x += positionArray[i].x;
      center.y += positionArray[i].y;
    }
    center.x /= positionArray.length;
    center.y /= positionArray.length;
    return center
  }
...
function createTrackers() {
  ...
  for(let i = 0; i < 3; i++) {
    ...
    trackingPoints[0].push(new tracker(color, i, cameractx, PI))
  }
  trackingPoints[1].push(new trackingDisplay())
}
...
function drawVideo(videoElement) {
  ...
  if(trackerSettings.VideoTracking) {
    ...
    trackingPoints[1][0].setPositions(
...
(async function renderLoop() {
  ...
  while (true) {
    ...
    if (now - then < trackerSettings.drawTimer - delta) {
        continue;
    }
    delta = Math.min(trackerSettings.drawTimer, delta + now - then - trackerSettings.drawTimer);
    then = now;
