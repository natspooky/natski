...
class cameraPoint {
  ...
  drawAnchor(){
    console.log(this.position, 'final')
    if(trackerSettings.ShowTrackingPoints && !isNaN(this.position.x || this.position.y)){
      this.ctx.beginPath();
  ...
  getPosition(videoframe, amount = trackerSettings.Threshold) {
    ...
    console.log(location, 'getpos')
    return location
  }

  match(videoCol, amount) {
    //console.log(this.space(videoCol) <= amount, this.space(videoCol), amount)
    return this.space(videoCol) <= amount;
  }
  ...
  space(col) {
    ...
    for(let i = 0; i < col.length; i++) {
      distance += (col[i] - this.color[i]) * (col[i] - this.color[i])
    }
