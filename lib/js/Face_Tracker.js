...
currentAudioMedia = undefined,
trackingPoints = [],
colourValues = [255,255,255]
videoConstraints = {video:{width: { max: 1280 },height: { max: 720 }, frameRate: { max: 30 }, deviceId:null}, audio:false},
audioConstraints = {audio: { noiseSuppression: false, echoCancellation: false, deviceId:null}},
trackerSettings = {
...
function changeInputData(value,deviceType,constraintType){
  switch(deviceType){
    case 'video':
      ...
      getMedia(videoConstraints, 'video')
      break;
    case 'audio':
      audioConstraints.video.deviceId = { exact: value }
      getMedia(audioConstraints, 'audio')
      break;
  }
...
function changeMicrophoneData(value){
...
async function getMedia(constraints, type){
  frameTimer = undefined
  switch(type){
    case 'video':
    ...
    case 'audio':
      stopMedia(currentAudioMedia)
      await navigator.mediaDevices
        .getUserMedia(constraints)
        .then(Info => {

          currentAudioMedia = Info;
          const audioContext = new AudioContext();
          const analyser = audioContext.createAnalyser();
          const microphone = audioContext.createMediaStreamSource(Info);
          const scriptProcessor = audioContext.createScriptProcessor(512, 1, 1);
    ...
    devices.forEach((device) => {
      ...
      if(device.kind == 'videoinput' || device.kind == 'audioinput'){
        ...
        item.addEventListener('click',function(){
          ...
          changeInputData(device.deviceId, inputType, 'device')
        })
        console.log(Select)
...
function setup(){
  ...
  getMedia(videoConstraints, 'video').then(() => {
    getSettingsData()
  })
...
class VideoProcessor {
  constructor(colour, colourIndex) {
...
class calibrator extends VideoProcessor {
  ...
  constructor(colour, colourIndex, ctx) {
    super(colour, colourIndex)
    this.frame = ctx.getImageData(0, 0, video.width, video.height)
  }
...
function startCalibrate() {
  ...
  for(let i = 0; i < 3; i++) {
    let colour = [0,0,0];
    colour[i] = 255;
    calibrators.push(new calibrator(colour, i, cameractx))
  }
  for(let i = 0; i < 3; i++) {
...
function createTrackers() {
  ...
  for(let i = 0; i < 3; i++) {
    ...
    trackingPoints.push(new tracker(color, i, cameractx, PI))
  }
}
