...
videoConstraints = {video:{width: { max: 1280 },height: { max: 720 }, deviceId: null}, audio:true}
=======
videoConstraints = {video:{width: { max: 1280 },height: { max: 720 }, frameRate: { max: 30 }}, audio:false,}
>>>>>>> parent of 36105f0 (N 2.0)
...
function changeCameraData(value,constraintType){
    ...
    switch(constraintType){
      ...
      case 'resolution':
        ...
        videoConstraints.video.width = { max: Number(tempValue[0]) }
        videoConstraints.video.height = { max: Number(tempValue[1]) }
        break;
    }
    console.log(value)
...
function getMedia(constraints){
  ...
  if(currentVideoMedia){
    console.log(currentVideoMedia.getTracks())
    currentVideoMedia.getTracks().forEach(track => {
      track.stop();
    });
  }
  frameTimer = undefined
...
function getPointCamera(videoframe, color, amount = threshold) {
  let location = [],
...
//add function to lock face to center, settings to change camera input FPS,  add setting to lock rotation.
  ...
  faceCtx.beginPath();
  faceCtx.moveTo(centerAnchor.x + xr, centerAnchor.y + yr);
  faceCtx.lineTo(centerAnchor.x + xr + xt, centerAnchor.y + yr + yt)
  faceCtx.arc(centerAnchor.x + xr + yt, centerAnchor.y + yr + yt, 0.05, 0, PI*2);
  faceCtx.stroke();
