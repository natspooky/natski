...
class CMS {
    ...
    fadeOut(event) {
        ...
        let distance = Math.sqrt((event.clientX - this.CMS.offsetLeft + (this.CMS.offsetWidth / 2)) ** 2 + (event.clientY - this.CMS.offsetTop + (this.CMS.offsetHeight / 2)) ** 2)

        if(distance >= 400 && !event.target.classList.contains('CMS')) {

            this.CMS.style.opacity = 1 - ((distance - 400) / 100)
...
console.log(this.pages)
        this.eventListeners.push(
            ...Array.prototype.map.call(this.pages, (element, index) => {
                ...
                if(index != 0) {
                    ...
                    return () => element.removeEventListener('click', clicker)

                }

            }).slice(1, this.pages.length)
