...
class CMS {
    ...
    fadeOut(event) {
        ...
        let distance = Math.sqrt((this.CMS.offsetLeft + (this.CMS.offsetWidth / 2) - event.clientX) ** 2 + (this.CMS.offsetTop + (this.CMS.offsetHeight / 2) - event.clientY) ** 2)

        if(distance >= 200 && !event.target.classList.contains('CMS')) {

            this.CMS.style.opacity = 1 - ((distance - 200) / 100)
...
/*
        this.pages.forEach((page, index) => {
            if(index != 0) {
                ...
                let clicker = () => this.changePage(page.getAttribute('CMSreturnPage'))
                
                page.addEventListener('click', clicker)

                this.eventListeners.push(() => page.removeEventListener(clicker))
            }
        })*/

        //alternate incase of falure
console.log(this.pages)

    Array.prototype.forEach.call(this.pages, (element, index) => {
               
        this.eventListeners.push(
             ...Array.prototype.map.call(element.children, (child, index) => {

                if(child.classList.contains('CMSclickable')) {
                    //console.log(child)
                    let clicker = () => {
                        //console.log('   clicked')
                        this.changePage(child.getAttribute('CMSreturnPage'))
                }

                    child.addEventListener('click', clicker)

                    return () => child.removeEventListener('click', clicker)
                }
            })
        )
...
*/
    ...
    attachToElement() {
        ...
        let attachment = (this.settings.attachment == null) ? document : document.querySelectorAll(this.settings.attachment),
        ...
        if(attachment.length > 1) {
            ...
            attachment[0].addEventListener('contextmenu', opener)

            this.eventListeners.push(

                () => attachment[0].removeEventListener('contextmenu', opener)
