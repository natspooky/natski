...
/* CORE INITIAL LOADER */
document.getElementById('headerimage').imagesLoaded()
.done(function() {
    if(preference!=='true') {
        setTimeout(() => {
    ...
    setTimeout(() => {
        ...
        $('.colourimage').imagesLoaded().done(function() {
            if(preference!=='true') {
                let colimg = document.getElementsByClassName('colourimage')
                for(let i = 0; i < colimg; i++) {
                    let theme = colourSystem(colimg[i]);
                    colimg.style.boxShadow = '0 0 40px ' + theme + ", 0.7)"
                    colimg.style.backgroundColor = theme + ", 0.7)"
                }
            }
        })
    }, 605);
