...
function checkChildrenGIS(element){
    if(element.tagName == 'GIS-ICON') {
        element.style.mask = 'url(https://natski.netlify.app/icon/svg/ENCORE_GIS/' + element.title + '.svg) no-repeat center'
        element.style.webkitMask = 'url(https://natski.netlify.app/icon/svg/ENCORE_GIS/' + element.title + '.svg) no-repeat center'
    }
    else if(element.length > 0) {
        checkChildrenGIS(element.childNodes)
    }
}
