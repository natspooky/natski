...
function cinemaVPS(e) {
    if(document.getElementsByClassName('VPScontainer')[e].classList.contains('full')) {
        document.getElementsByClassName('VPScontainer')[e].classList.remove('full')
    }else{
        document.getElementsByClassName('VPScontainer')[e].classList.add('full')
        //document.getElementsByClassName('VPScontainer')[e].style.height = '100vh'
        //document.getElementsByClassName('VPScontainer')[e].style.width = '100vw'
        //document.getElementsByClassName('VPScontainer')[e].style.maxHeight = '100vh'
    }
}
    ...
    for(let x = 0; x < 5; x++){
        ...
        if(x == 1 || x == 3){
            if(!device){
                ...
                controlButtons.title = buttonTitle[x]
...
function addVPSfuntions(e,device) {
    ...
    //video.onloadstart = function(){bufferVPS(e)}
    video.onplay = function(){currentVideoTimeCompleteVPS(e); document.getElementsByClassName('VPSbar')[e].style.display = 'block'}
    ...
    if(!device){
        ...
        proginput.ontouchend = function(){overlayDisplayVPS(e)}
    }
    ...
    playback.onclick = function(){toggleSettingsPageVPS(e,document.getElementsByClassName('VPSplaybackMenu')[e]);clearTimeout(VPStimer)}
    container.onmousemove = function(){overlayDisplayVPS(e)}
}

function loadVPS() {
    let length = document.getElementsByClassName('VPS').length,
    device = deviceCheckVPS();
    for(let i = 0; i < length; i++){
