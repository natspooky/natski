...
function mute(e) {
    if(document.getElementsByClassName('VPS')[e].volume != 0){
        adjustVolume(e, 0)
        document.getElementsByClassName('VPSsound')[e].classList.add('muted')
    }else{
        adjustVolume(e, 100)
    }
}
...
function VPSloop(e){
    if(document.getElementsByClassName('VPS')[e].loop){
        document.getElementsByClassName('VPS')[e].removeAttribute('loop')
    }else{
        document.getElementsByClassName('VPS')[e].setAttribute('loop', '')
    }
    document.getElementsByClassName('VPSloop')[e].classList.toggle('active')
}

function adjustBar(x,e) {
    document.getElementsByClassName('VPSbarChange')[x].style.width = e + '%'
}

function adjustInputbar(x,e) {
    document.getElementsByClassName('VPSinput')[x].value = e
}

function adjustVolume(x,e) {
    volumeSVGactive(x)
    document.getElementsByClassName('VPSsound')[x].classList.remove('muted')
    document.getElementsByClassName('volumeVisible')[x].style.width = e + '%'
    document.getElementsByClassName('VPS')[x].volume = e / 100
    if(document.getElementsByClassName('VPS')[x].volume == 0){
        document.getElementsByClassName('VPSsound')[x].classList.add('muted')
        volumeSVGdeactive(x)
    }
}
...
function toggleSettings(e) {
    ...
    document.getElementsByClassName('VPSsettings')[e].classList.toggle('setting')
}

function videoTime(x,e) {
...
function currentVideoTime(e) {
    let [min,sec] = formatTime(document.getElementsByClassName('VPS')[e].currentTime);
    document.getElementsByClassName('timeCurrent')[e].innerText = min + ':' + sec
}

function currentVideoTimeComplete(e) {
    currentVideoTime(e)
    let [endmin,endsec] = formatTime(document.getElementsByClassName('VPS')[e].duration)
    document.getElementsByClassName('timeEnd')[e].innerText = '/' + endmin + ':' + endsec
}
...
function pause(e) {
    ...
    video.pause()
    document.getElementsByClassName('VPSplay')[e].classList.remove('active')
}
...
function playPause(e) {
    ...
    if(video.paused){
        video.play()
        document.getElementsByClassName('VPSplay')[e].classList.add('active')
    }else{
        video.pause()
        document.getElementsByClassName('VPSplay')[e].classList.remove('active')
    }
...
function fullScreen(e) {
    ...
    if(document.fullscreenElement || document.webkitFullscreenElement || document.mozFullScreenElement){
        ...
        document.getElementsByClassName('VPS')[e].classList.remove('full')
    }else{
        if (video.requestFullscreen) {
            video.requestFullscreen();
        }
        else if (video.children[0].webkitEnterFullscreen) { 
        ...
        document.getElementsByClassName('VPS')[e].classList.add('full')
    }
}

function createVPSelements(e) {
    let device = deviceCheck();
    wrapVPSvideo(e)
    createVPScontrols(e, device)
    addVPSfuntions(e, device)
}
...
function createVPScontrols(e,device) {
    ...
    progbarinput.className = 'VPSinput'
    ...
    if(!device){
        ...
        volumeInput.className = 'volumeInput'
    ...
    settings.appendChild(settingsloop)
    for(let x = 0; x < 4; x++){
...
function addVPSfuntions(e,device) {
    let video = document.getElementsByClassName('VPS')[e],
    ...
    contentbar = document.getElementsByClassName('VPSbar')[e],
    settings = document.getElementsByClassName('VPSsettings')[e];
    video.ontimeupdate = function(){
        let percent = (video.currentTime / video.duration) * 100
        adjustBar(e, percent);
        adjustInputbar(e, percent)
        currentVideoTime(e)
    }
    video.onended = function(){showCont(e);document.getElementsByClassName('VPSplay')[e].classList.remove('active')}
    video.onpause = function(){showCont(e)}
    video.onplay = function(){hideCont(e)}
    video.onwaiting = function(){buffer(e)}
    video.onplaying = function(){bufferClear(e)}
    video.onplay = function(){currentVideoTimeComplete(e)}
    video.onpause = function(){pause(e)}
    video.onloadstart = function(){buffer(e)}
    playframe.onclick = function(){playPause(e)}
    proginput.oninput = function(){adjustBar(e,proginput.value);videoTime(e,proginput.value);currentVideoTime(e);currentVideoTimeComplete(e)}
    player.onclick = function(){playPause(e)}
    fullsc.onclick = function(){fullScreen(e)}
    if(!device){
        volume.oninput = function(){adjustVolume(e,volume.value);}
        volumecont.onclick = function(){mute(e)}
        volumecont.onmouseenter = function(){openVolume(e)}
        contentbar.onmouseleave = function(){closeVolume(e)}
        volumebox.onmouseenter = function(){openVolume(e)}
    }
    settings.onclick = function(){toggleSettings(e)}
    loop.onclick = function(){VPSloop(e)}
    container.onmousemove = function(){overlayDisplay(e)}
    container.onmouseleave = function(){hideCont(e)}
}

function loadVPS() {
    for(let i = 0; i < document.getElementsByClassName('VPS').length; i++){
        createVPSelements(i)
    }
}
...
function VPSstyleCall(url) {
    VPScall.open('GET', url);
    VPScall.onloadend = function(){VPSstyler()};
    VPScall.send();
}
...
if(checkLoadedVPS() == true) {
    VPSstyleCall('https://natski.netlify.app/lib/ENCORE_DB/VPS/'+VPS_settings.style+'.txt')
}
