...
function togglePlaybackMenu(e) {
    ...
    document.getElementsByClassName('VPSplaybackMenu')[e].classList.toggle('active')
}
...
function setPlayback(e,x) {
    togglePlaybackMenu(e)
    document.getElementsByClassName('VPS')[e].playbackRate = x.innerText
    document.getElementsByClassName('VPSplayback')[e].innerText = 'playback: ' + x.innerText + 'x'
}
...
function createVPScontrols(e,device) {
    ...
    settingsplayback.innerHTML = 'playback: 1x'
    playbackmenu.className = 'VPSplaybackMenu'
    settings.appendChild(settingsplayback)
    ...
    for(let x = 0; x < 4; x++){
        ...
        playbackbut.innerText = (x+1)*0.5
        playbackbut.onclick = function(){setPlayback(e,this)}
    ...
    controlbox.appendChild(playbackmenu)
...
function addVPSfuntions(e,device) {
    ...
    proginput.oninput = function(){
        adjustBar(e,proginput.value);
        videoTime(e,proginput.value);
        currentVideoTime(e);
        currentVideoTimeComplete(e);
        showCont(e)
    }
    ...
    if(!device){
        ...
        volumecont.onclick = function(){mute(e);showCont(e)}
    ...
    settings.onclick = function(){toggleSettings(e);showCont(e);clearTimeout(VPStimer)}
    loop.onclick = function(){VPSloop(e);clearTimeout(VPStimer)}
    playback.onclick = function(){togglePlaybackMenu(e);clearTimeout(VPStimer)}
