...
function toggleSettings(e) {
    if(document.getElementsByClassName('VPSsettings')[e].classList.contains('setting') && !document.getElementsByClassName('VPS')[e].paused){
        setTimeout(() => {
            hideCont(e)
        }, 300);
    }
...
function currentVideoTime(e) {
    let video = document.getElementsByClassName('VPS')[e],
    [min,sec] = formatTime(video.currentTime);
...
function currentVideoTimeComplete(e) {
    currentVideoTime(e)
    let video = document.getElementsByClassName('VPS')[e],
    [min,sec] = formatTime(video.duration);
...
function toggleSettingsPage(e,x) {
...
function wrapVPSvideo(e) {
    ...
    videoSrc = video.children[0].src;
    container.className = 'VPScontainer'
    container.style.width = video.getAttribute('VPSwidth')
    video.children[0].type = 'video/' + videoSrc.substring(videoSrc.length - 3, videoSrc.length)
...
function createVPScontrols(e,device) {
    ...
    //make container for main settings
    ...
    for(let x = 0; x < 4; x++){
        ...
        playbackbut.onclick = function(){setPlayback(e,this);toggleSettingsPage(e,document.getElementsByClassName('VPSplaybackMenu')[e]);}
...
function loadsettingsmenu(e) {
    document.getElementsByClassName('settingsPanel')[e].style.width = document.getElementsByClassName('VPSmainSettings')[e].offsetWidth + 'px'
    document.getElementsByClassName('settingsPanel')[e].style.height = document.getElementsByClassName('VPSmainSettings')[e].offsetHeight + 'px'
}
...
function addVPSfuntions(e,device) {
    ...
    video.onplay = function(){currentVideoTimeComplete(e); document.getElementsByClassName('VPSbar')[e].style.display = 'block'}
    ...
    if(!device){
        ...
        proginput.onchange = function(){document.getElementsByClassName('VPSplay')[e].style.opacity = '1'}
    }
    settings.onclick = function(){toggleSettings(e);showCont(e)}
    loop.onclick = function(){VPSloop(e);clearTimeout(VPStimer)}
    playback.onclick = function(){toggleSettingsPage(e,document.getElementsByClassName('VPSplaybackMenu')[e]);clearTimeout(VPStimer)}
