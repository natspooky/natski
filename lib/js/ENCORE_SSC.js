...
function createSSCElements(settings, device) {
    ...
    if(settings.progressBar) {
        ...
        progresscont.className = 'SSCprogress'
        progressbar.className = 'SSCprogressBar'
...
class SSC {
    ...
    elementCreator(elementType, elementClass, ariaLabel, eventType, eventFunction, functionVariable, passive) {
        ...
        if(eventType != null) {
            element.addEventListener(
                eventType, 
                (functionVariable != null) ? function(){ eventFunction(functionVariable) } : eventFunction,
                passive
                )
        }
        return element
    ...
    createElements() {
        if(this.settings.thumbs) {
            (() => {
                let thumbcont = this.elementCreator('div', 'SSCthumbs', null, null, null, null, null);
                for(let i = 0; i < this.pages.length; i++) {
                    thumbcont.appendChild(this.elementCreator('div', 'SSCthumb', null, 'click', this.directPage, (i + 1), true))
                }
        ...
        if(this.settings.progressBar) {
            (() => {
                let progcont = this.elementCreator('div', 'SSCprogress', null, null, null, null, null),
                prog = this.elementCreator('div', 'SSCprogressBar', null, null, null, null, null)
                this.SSC.appendChild((progcont.appendChild(prog)))
                return Promise.resolve(0)
            })()
            .then(() => {
                this.progressBar = this.SSC.getElementsByClassName('SSCprogressBar')
            })
        }
        if(this.settings.sideButtons && ! this.device) {
            this.SSC.appendChild(
                (this.elementCreator('button', 'SSCpageButton', 'Previous Page', 'click', this.changePage, -1, true).innerHTML = SSCicons['leftArrow'])
            )
            this.SSC.appendChild(
                (this.elementCreator('button', 'SSCpageButton', 'Next Page', 'click', this.changePage, 1, true).innerHTML = SSCicons['rightArrow'])
            )
        }
        if(this.settings.playButton) {
