...
function SSCstyleCall(url, settings) {
    SSCcall.open('GET', url);
    SSCcall.setRequestHeader("Content-Type", "text/plain");
    SSCcall.setRequestHeader("Access-Control-Allow-Origin", "*");
    SSCcall.onloadend = function(){SSCstyler(settings)};
    SSCcall.send();
}
...
if(checkLoadedSSC() == true) {
    SSCstyleCall('https://natski.netlify.app/lib/ENCORE_DB/SSC/'+SSC_settings.style+'.txt', SSC_settings)
}
