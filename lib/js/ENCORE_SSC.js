...
class SSC {
    ...
    pause() {
        ...
        if(this.settings.pauseButton){this.pauseButton.style.classList.add('paused')}
    }

    play() {
        this.paused = false
        this.setTimers()
        if(this.settings.pauseButton) this.pauseButton.style.classList.remove('paused')
    }
    ...
    createElements() {
        ...
        if(this.settings.pauseButton) {
            (() => {
                let play = this.elementCreator('button', 'SSCpauseButton', 'Play / Pause', 'click', this.togglePlay.bind(this), 0)
                play.innerHTML = `${SSCicons['pause']} ${SSCicons['play']}`
                this.SSC.appendChild(play)
                return Promise.resolve(0)
            })()
            .then(() => {
                this.play = this.SSC.getElementsByClassName('SSCpauseButton')[0]
            })
        }
