...
function reloadSSC(settings) {
    ...
    for(let i = 0; i < sscVersions.length; i++) {
        let len = sscVersions[i].childElementCount - sscVersions[i].getElementsByClassName('banner-bg').length
        for(let y = 0; y < len.length; i++) {
            sscVersions[i].children[sscVersions[i].childElementCount -1].remove()
        }
    }
    loadSSC(settings)
}
...
class SSC {
    ...
    displayPage() {
        ...
        if(this.settings.thumbs) {
            ...
            this.thumbs[this.index-1].classList.add('active')
        }
        this.pages[this.index-1].style.display = 'flex'
        this.pages[this.index-1].style.opacity = '0'
        Promise.resolve(this.pages[this.index-1].style.transition = 'opacity 0s').then(() => {
            this.pages[this.index-1].style.opacity = '1'
            this.pages[this.index-1].style.transition = 'opacity 0.1s'
        })
    }
    ...
    pause() {
        ...
        if(this.settings.progressBar) clearInterval(this.barTimerFunc)
        if(this.settings.playButton) this.playButton.classList.add('paused')
    ...
    play() {
        this.paused = false
        this.setTimers()
        if(this.settings.playButton) this.playButton.classList.remove('paused')
    ...
    createElements() {
        ...
        if(this.settings.playButton) {
            (() => {
                let play = this.elementCreator('button', 'SSCplayButton', 'Play / Pause', 'click', this.togglePlay.bind(this), null)
                play.innerHTML = `${SSCicons['pause']} + ${SSCicons['play']}`
                this.SSC.appendChild(play)
                return Promise.resolve(0)
            })()
            .then(() => {
                this.play = this.SSC.getElementsByClassName('SSCplayButton')[0]
            })
        }
        return Promise.resolve(0)
    }
    ...
    swipeSystem() {
        if(this.device) {
            ...
            this.SSC.addEventListener("touchmove", (e) => {
                ...
                this.pages[this.index-1].style.opacity = `${1 - (Math.min(Math.abs(this.touch - SSCtouchMove),150) / 150)}`
    ...
    loadSlideshow() {
        ...
        this.SSC.style.transition = 'opacity 0.5s'
