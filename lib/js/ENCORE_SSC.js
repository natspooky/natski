...
document.getElementById('SSC').style.transition = '0s'
document.getElementById('SSC').style.opacity = '0'
...
window.addEventListener("load", function() {
    ...
    let slideshowContainer = document.getElementsByClassName("banner-button");
...
function slideShowElements(settings) {
    SSCstyleCall('https://natski.netlify.app/lib/ENCORE_DB/SSC/'+settings.style+'.txt')
...
const SSCcall = new XMLHttpRequest();
const SSCstyler = function() {
    setTimeout(() => {
        document.getElementById('SSC').style.transition = '0.6s'
        document.getElementById('SSC').style.opacity = '1'
    }, 100)
    let style = document.createElement('style')
    style.innerHTML = SSCcall.responseText;
    document.getElementById('SSC').appendChild(style)
}


function SSCstyleCall(url) {
    SSCcall.open('GET', url);
    SSCcall.onloadend = SSCstyler;
    SSCcall.send();
}
