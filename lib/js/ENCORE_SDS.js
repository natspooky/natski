...
.done(function() {
        setTimeout(() => {
            ...
            document.getElementById('inspectPanel').style.backgroundColor = colourSystemAlpha($('#inspectImage')) + "0.8)"
        }, 100);
})
})
...
function inspectLoad() {
    document.getElementById('inspectSVG').style.transition = '0s'
    document.getElementById('inspectImage').style.transition = '0s'
    document.getElementById('inspectSVG').style.opacity = '1'
    document.getElementById('inspectImage').style.opacity = '0'
    $('#inspectImage').imagesLoaded()
    .done(function() {
        setTimeout(() => {
            theme = colourSystemAlpha($('#inspectImage'))
            document.getElementById('inspectSVG').style.transition = '0.4s'
    document.getElementById('inspectImage').style.transition = '0.4s'
            document.getElementById('inspectSVG').style.opacity = '0'
            document.getElementById('inspectImage').style.opacity = '1'
...
function inspectChange(path, indicator) {
    if(imageIndex + indicator < 1 || imageIndex + indicator > document.getElementsByClassName('colourimage').length -1){
        return
    }else{
        imageIndex += indicator
