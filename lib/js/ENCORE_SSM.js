...
function popupSystemSSM(popup, position){
    ...
    popup.style.transform = `translate(${position.clientX + 20}px,${position.clientY + 20}px)`
...
function searchSSM(e,settings){
    ...
    for(let i = 0; i < len; i++){
        ...
        console.log(`${text} in ${(settings[i].children[0].innerText).toLowerCase()} = ${((settings[i].children[0].innerText).toLowerCase()).includes(text)}`)
    }
