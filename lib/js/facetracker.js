...
/* variables */
const cameracan = document.getElementById('video'), canvas = document.getElementById('canvas'), ctx = canvas.getContext("2d"), cameractx = cameracan.getContext('2d'),
anchors={
  ...
  leftAnchor:{x:-0.5,y:0,xRange:[0,0.2],yRange:[0,0.3]},
  rightAnchor:{x:0.5,y:0,xRange:[0,0.2],yRange:[0,0.3]}*/
},
{centerAnchor, topAnchor, bottomAnchor} = anchors;
...
function drawhead() {
  ...
  ctx.quadraticCurveTo(0.4, 0.5,bottomAnchor.x,bottomAnchor.y);
  ctx.stroke();
...
function drawnose(point) {
  ...
  ctx.beginPath();
  ctx.moveTo(0,0);
  ctx.quadraticCurveTo((point.x*1.05),(point.y/2)+0.15,0,0.25);
  ctx.stroke();
  ctx.restore();
};
...
function getPointCamera(videoframe,color=[0,0,255],amount=150){
  const location = [];
...
function frames() {
  ...
  if (video) {
    ...
    if(videolocation.length>0){
      ...
      drawcameraanchor(average)
      moveAnchor({value:Xmap*9},'x');
...
function drawcameraanchor(location,rad=5) {
  cameractx.beginPath();
