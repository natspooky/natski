...
/* variables */
var mouthnum, sidemouth, volume = 0.05;
let video = null;
...
navigator.mediaDevices.getUserMedia({
  ...
  scriptProcessor.onaudioprocess = function() {
    ...
    document.getElementById('bar').style.transform = 'scaleY('+average/100+')'
...
function drawhead() {
  ...
  drawmouth(centerAnchor,mouthnum,sidemouth);
...
function drawmouth(point,mouthmove,sidemouth) {

  ctx.save();
  ctx.translate(point.x/2,(point.y/2)+0.3);

  ctx.beginPath();
  ctx.moveTo(-0.2+sidemouth,0);
  ctx.quadraticCurveTo(0,(point.y/2.3)+mouthmove,0.2-sidemouth,0);
  ctx.quadraticCurveTo(0,(point.y/2.3)-mouthmove,-0.2+sidemouth,0);
  ctx.stroke();
  ctx.restore();
