...
//make new loading system to allow for queries and extra tags to make this system compatable with or without the implementation of the window system
...
if (
	...
	if (window.location === window.parent.location) {
		window.location.href = `${window.location.href.slice(
			0,
			window.location.href.lastIndexOf('080/') + 1 + 3,
...
function createLayout() {
	let route = window.location.href
			.slice(
				window.location.href.lastIndexOf('080/') !== -1
					? window.location.href.lastIndexOf('080/') + 4
					: 0,
