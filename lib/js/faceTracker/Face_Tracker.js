...
faceCtx = output.getContext('2d', {willReadFrequently: true}),
...
function setup(){
  output.height = 400
  output.width = 400
  faceCtx.translate(output.width/2,output.height/2);
  faceCtx.scale(output.width*0.5,output.height*0.5);
...
class VideoProcessor {
  ...
  convertLabToRGB(lab){
    ...
    red = (red > 0.0031308) ? (1.055 * (r ** (1/2.4)) - 0.055) : 12.92 * red;
    green = (green > 0.0031308) ? (1.055 * (g ** (1/2.4)) - 0.055) : 12.92 * green;
    blue = (blue > 0.0031308) ? (1.055 * (b ** (1/2.4)) - 0.055) : 12.92 * blue;

    return [APF.max([0, APF.min([1, r])]) * 255, 
            APF.max([0, APF.min([1, g])]) * 255, 
            APF.max([0, APF.min([1, b])]) * 255]
  }
...
class tracker extends VideoProcessor {
  ...
  constructor(colour, colourIndex, ctx, PI) {
    ...
    this.position = {x:0,y:0}
    //console.log(colour)
    this.fillCol = `rgb(${colour[0]},${colour[1]},${colour[2]})`// make these colours RBG again
    this.diameter = PI * 2
  }
    ...
    if(!trackerSettings.LocationLock) {
      for(let ID in this.anchors) {
        ...
        if(ID == 'center') {
          ...
          coords = {
          x: this.anchors[ID].x + ((this.positions[2].x - this.center.x) / canvas.width) + this.interpolate(this.range.x[0], this.range.x[1], Xmap), 
          y: this.anchors[ID].y + ((this.positions[2].y - this.center.y) / canvas.height) + this.interpolate(this.range.y[0], this.range.y[1], Ymap)}

        }else {
      
          let centerPoint = {
          x: this.anchors['center'].x -((canvas.height / 2 - this.center.x) / canvas.height) + this.interpolate(this.range.x[0], this.range.x[1], Xmap), 
          ...
          x: this.anchors[ID].x -((canvas.height / 2 - this.center.x) / canvas.height) + this.interpolate(this.range.x[0], this.range.x[1], Xmap),
      ...
      for(let ID in this.anchors) {
        ...
        if(ID == 'center') {
          ...
          coords = {
          x: this.anchors[ID].x + ((this.positions[2].x - this.center.x) / canvas.width) + this.interpolate(
            this.range.x[0], 
            this.range.x[1], 
            Xmap), 
          y: this.anchors[ID].y + ((this.positions[2].y - this.center.y) / canvas.height) + this.interpolate(
            this.range.y[0], 
            this.range.y[1], 
            Ymap)}
...
function changethresh(e, val) {
  trackerSettings.Threshold[val] = (4 * parseInt(e.value))
}
...
function labDeltaE(labOne, labTwo){
  ...
  deltaB = labOne[2] - labTwo[2],
  c1 = APF.squareRoot(labOne[1] * labOne[1] + labOne[2] * labOne[2]),
  c2 = APF.squareRoot(labTwo[1] * labTwo[1] + labTwo[2] * labTwo[2]),
  deltaC = c1 - c2,
  deltaH = deltaA * deltaA + deltaB * deltaB - deltaC * deltaC;
  deltaH = deltaH < 0 ? 0 : APF.squareRoot(deltaH);
  let sc = 1.0 + 0.045 * c1,
