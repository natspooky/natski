...
class trackingDisplay {
  ...
  calculateRotationAngle() {
    ...
    this.angle = APF.inverseTan(this.positions[1].y - this.center.y, this.positions[1].x - this.center.x)
  ...
  rotationMatrix() {
    let points = {x: point - this.center.x, y: point - this.center.y}
    APF.multiplyMatrix([[APF.cos(this.angle), -(APF.sin(this.angle))], [APF.sin(this.angle), APF.cos(this.angle)]],[points.x, points.y])
  ...
  drawFaceCanvas() {
    ...
    for(let ID in this.anchors) {
      ...
      if(ID == 'center') {
        ...
        y = this.anchors[ID].y -((canvas.width / 2 - this.center.y) / canvas.width) + this.interpolate(this.range.y[0], this.range.y[1], Ymap)
      }
      //this.rotationMatrix()
      ...
      if(trackerSettings.ShowTrackingPoints){
        ...
        this.outputCtx.arc(x, y, 0.02, 0, this.diameter);
