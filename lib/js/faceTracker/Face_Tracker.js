...
trackingPoints = [[],[]],
colourValues = [[230,0,0],[0,230,0],[0,0,230]]
...
class VideoProcessor {
  ...
  convertRGBToLab(rgb){
    ...
    red = (red > 0.04045) ? APF.squareRoot((red + 0.055) / 1.055) ** 5 : red / 12.92;
    green = (green > 0.04045) ? APF.squareRoot((green + 0.055) / 1.055) ** 5 : green / 12.92;
    blue = (blue > 0.04045) ? APF.squareRoot((blue + 0.055) / 1.055) ** 5 : blue / 12.92;
    ...
    x = (x > 0.008856) ? APF.cubeRoot(x) : (7.787 * x) + 0.13793;
    y = (y > 0.008856) ? APF.cubeRoot(y) : (7.787 * y) + 0.13793;
    z = (z > 0.008856) ? APF.cubeRoot(z) : (7.787 * z) + 0.13793;
  ...
  labDeltaE(labOne, labTwo) {
    ...
    c1 = APF.squareRoot(labOne[1] ** 2 + labOne[2] ** 2),
    deltaC = c1 - APF.squareRoot(labTwo[1] ** 2 + labTwo[2] ** 2),
  ...
  match(videoColor, Threshold) {
    ...
    return this.labDeltaE(this.colour, this.convertRGBToLab(videoColor)) <= Threshold;
  }
/*
  space(colour) {
    let distance = 0;
