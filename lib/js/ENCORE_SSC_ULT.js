...
function reloadSSCU(settings) {
    ...
    let len = document.getElementsByClassName('SSCU')[0].childElementCount - banners
    for(let i = 0; i < len; i++){
        document.getElementsByClassName('SSCU')[0].children[document.getElementsByClassName('SSCU')[0].childElementCount -1].remove()
    }
...
function loadSSCU(settings) {
    ...
    for(let i = 0; i < SSCUlength; i++){
        let count = SSCUbanner[i].childElementCount
        for(let x = 0; x < count; x++){
            if(SSCUbanner[i].children[x].tagName == 'IMG'){
                SSCUbanner[i].children[x].loading = 'eager'
                SSCUbanner[i].children[x].draggable = false
            }
        }
    ...
    if(SSCUlength < 2) {
        ...
        createSSCUElements(settings, device)
        currentSSCU(1)
    }
    if(device){
        ...
        SSCU.addEventListener("touchmove", function(e){
            ...
            if(Math.abs(SSCUtouchMove - SSCUtouchStart) > 15 && e.cancelable){
                e.preventDefault()
                SSCUscrollLock = true
            }else{
...
function createSSCUElements(settings, device) {
    let slideshowcontainer = document.getElementsByClassName('SSCU')[0];
    if(settings.thumbs) {
