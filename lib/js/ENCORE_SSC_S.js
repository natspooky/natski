...
function displaySSCS(e) {
    ...
    SSCS = document.getElementById('SSCS'),
    ...
    setTimeout(() => {
        if(pages[SSCSIndex-1].offsetHeight < thumbs[0].offsetHeight){
            SSCS.style.height = thumbs[0].offsetHeight + 'px'
        }else{
            SSCS.style.height = pages[SSCSIndex-1].offsetHeight + 'px'
        }
        pages[SSCSIndex-1].style.opacity = '1'
        pages[loopStorage].style.opacity = '0'
